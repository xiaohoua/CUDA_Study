// https://github.com/wangxu989/study/blob/main/c%2B%2B/cuda/algorithm/RadixSort.cu
#include<iostream>
#include<ctime>
#include<assert.h>
#include<algorithm>
...
void __global__ RadixSort(float* data, unsigned int* sort_tmp0,unsigned int* sort_tmp1, unsigned int n) {
  ...
  for(unsigned int i = 0;i < n;i++) {
    ...
    if(tid == 0 && i==0)printf("elem = %u\n", elem);
    __syncthreads();
    min_value = min_tid = 0xffffffff;
    atomicMin(&min_value,elem);
    __syncthreads();
    if(tid == 0 && i==0)printf("min_val = %u\n", min_value);
    if(min_value == elem) {
      atomicMin(&min_tid, tid);
    }
    __syncthreads();
    if(tid == 0 && i==0)printf("min_tid = %u\n", min_tid);
    if(min_tid == tid) {
      list_idx[tid]++;
