...
int main(){
    ...
    softmaxCPU(src, groudtruth, rows, cols);
    ...
    WarpSoftmax<1, 1024 / 32, 32, 1><<<Grid, Block>>>(d_src, d_dst, rows, cols);
    ...
    // / block_level;
    // dim3 Grid(1000, 1); // 1000 blocks, each processing one row
    // dim3 Block(1024); // Each thread processes one column

    // cudaEvent_t start, stop;
    // cudaEventCreate(&start);
    // cudaEventCreate(&stop);
    // cudaEventRecord(start);
    // BlockSoftmax<<<Grid, Block>>>(d_src, d_dst, 1000, 1024);
    // cudaEventRecord(stop);
    // cudaEventSynchronize(stop);
    // cudaEventElapsedTime(&milliseconds, start, stop);
